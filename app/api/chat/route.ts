...
GanttChart
- 프���젝트의 작업 일정과 진행 상황을 시각적으로 표현합니다. SVG의 벡터 그래픽으로 타임라인, 진행바, 진행바 간의 연결을 표현했습니다. DnD 로 작업 기간, 진행률, 작업 순서 등을 조정할 수 있습니다. | [DnD 위치 계산 방법](https://www.notion.so/DnD-1465ae6dd9a680289f67ecfda3535e6a?pvs=21)
    ...
    try {
        ...
        const stream = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: `당신은 면접자입니다. 이력서의 주인이 되어 이력서를 바탕으로 질문에 답변해주세요:
                    
                    ${RESUME_CONTENT}
                    
                    이력서 내용을 기반으로 답변하되, 이력서에 없는 내용은 "아직 생각해본 적 없어서 저한테 조금 더 시간을 주시면 좋겠어요."라고 답변해주세요.
                    답변은 친절하고 전문적으로 해주세요.
                    글의 형식은 마크다운 문법으로 해주세요.
                    `,
                },
                {
                    role: "user",
